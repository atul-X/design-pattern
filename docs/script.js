document.addEventListener('DOMContentLoaded', function() {
    ...
    document.querySelectorAll('.pattern-section, .demo-card, .code-example, .pattern-preview-grid, .hero-section h1, .hero-section p, .hero-section .d-flex, .stats-row').forEach(el => {
        observer.observe(el);
    });
    ...
    function updateConfig() {
        ...
        const logger = Logger.getInstance();
        logger.log('INFO', `Configuration updated: DB URL set to ${dbUrl}`);
    ...
    function verifySingleton() {
        ...
        document.getElementById('hash1').textContent = instance1.getInstanceId();
        document.getElementById('hash2').textContent = instance2.getInstanceId();

        const areSame = instance1 === instance2;
        const identityCheck = document.getElementById('identityCheck');
        identityCheck.textContent = areSame ? 'Yes, they are the same instance.' : 'No, different instances!';
        identityCheck.className = `identity-check ${areSame ? 'success' : 'error'}`;
    }
    ...
    function verifyLogger() {
        ...
        const verificationResult = document.getElementById('loggerVerification');
        const areSame = logger1 === logger2;

        if (areSame) {
            verificationResult.innerHTML = `
                <span class="text-success">Verification Successful!</span><br>
                <small>Instance 1 Hash: ${logger1.getInstanceId()}</small><br>
                <small>Instance 2 Hash: ${logger2.getInstanceId()}</small><br>
                <small>Both variables point to the same logger instance.</small>
            `;
        } else {
            verificationResult.innerHTML = `
                <span class="text-danger">Verification Failed!</span><br>
                <small>Instance 1 Hash: ${logger1.getInstanceId()}</small><br>
                <small>Instance 2 Hash: ${logger2.getInstanceId()}</small><br>
                <small>Different logger instances were created.</small>
            `;
        }
    }
    ...
    function executeCommand(commandType, param) {
        ...
        switch (commandType) {
            case 'lightOn':
                command = new LightCommand(smartHomeController.devices[param === 'livingRoom' ? 'livingRoomLight' : 'bedroomLight'], 'on');
                break;
            case 'lightOff':
                command = new LightCommand(smartHomeController.devices[param === 'livingRoom' ? 'livingRoomLight' : 'bedroomLight'], 'off');
                break;
            case 'lightDim':
                command = new LightCommand(smartHomeController.devices[param === 'livingRoom' ? 'livingRoomLight' : 'bedroomLight'], 'dim');
                break;
            case 'setTemp':
    ...
    function executeGoodNightMacro() {
        const macro = new MacroCommand([
            ...
            new LightCommand(smartHomeController.devices.bedroomLight, 'dim'),
            new SecurityCommand(smartHomeController.devices.security, 'arm'),
            new ThermostatCommand(smartHomeController.devices.thermostat, 18)
        ]);
        smartHomeController.executeCommand(macro);
    }

    function refreshDevices() {
        Object.values(smartHomeController.devices).forEach(device => device.updateUI());
        Logger.getInstance().log('INFO', 'Device statuses refreshed.');
    }
    ...
    class RoadLogistics extends Logistics {
        createTransport(type) { return new Truck(); }
    }

    class SeaLogistics extends Logistics {
        createTransport(type) { return new Ship(); }
    }
    ...
    function planDelivery() {
        ...
        const planResult = logistics.planDelivery(deliveryType);
        const transport = logistics.createTransport(deliveryType); // Get transport details for UI
    ...
    window.verifySingleton = verifySingleton;
    window.sendLogMessage = sendLogMessage;
    window.clearLog = clearLog;
    window.verifyLogger = verifyLogger;
    window.executeCommand = executeCommand;
    window.undoCommand = undoCommand;
    window.executeGoodNightMacro = executeGoodNightMacro;
    window.refreshDevices = refreshDevices;
    window.clearHistory = clearHistory;
    window.updateTempDisplay = updateTempDisplay;
    window.generateUI = generateUI;
    window.planDelivery = planDelivery;
});
